...
class JSONReader(object):
    def __init__(self, room_list, verbose=False):
        self.rooms = []
        self.victims = []
        self.fov_messages = []
        ...
        self.typeToFields = {
                        'Event:Triage':['triage_state', 'color'], 
                        'Event:RoleSelected': ['new_role', 'prev_role'], 
                        'Event:ToolDepleted': ['tool_type'], 
                        'Event:VictimPlaced': ['color'], 
                        'Event:VictimPickedUp': ['color'], 
                        'Event:ToolUsed': [],
                        'Event:Location': [],
    ...
    def process_message(self, jmsg):
        ...
        if mtype == "Event:MissionState":
            mission_state = jmsg['data']['mission_state']
            self.mission_running = (mission_state == "Start")
            print('mission ', mission_state, jmsg)
            return
                
        if mtype == "Event:Pause":
            isPaused = jmsg['data']['paused']
