...
class ProcessParsedJson(GameLogParser):
    ...
    def parseMove(self, player, newRoom, msgIdx, ts):
        ...
        if self.lastParsedLoc == None:
            ...
            self.logger.debug('%s moved to %s at %s' % (player, self.lastParsedLoc, ts))
            return 0
        ...
        if INJECT_PSYCH_ACTIONS:
            ...
            mv = [True]
        if mv == []:
            self.logger.error('%s unreachable %s to %s at %s' % (player, self.lastParsedLoc, newRoom, ts))
            self.lastParsedLoc = newRoom
            return 1

        if len(mv) > 1:
            self.logger.error('%s multiple steps from %s to %s at %s' % (player, self.lastParsedLoc, newRoom, ts))
        for mAct in mv:
...
#                input('press any key.. ')
            ...
            if mtype == 'Event:Triage':
                ...
                if triageInProgress:
                    ...
                    triageInProgress = False
                loc = m['room_name']
                err = self.parseMove(m['playername'], loc, numMsgs, ts)
                    
            elif mtype == 'Event:VictimPickedUp':
                self.logger.info(m['playername'] + " picked " + vicColor + " in " + self.lastParsedLoc)
                self.parseVictimPicked(vicColor, self.lastParsedLoc, ts)
