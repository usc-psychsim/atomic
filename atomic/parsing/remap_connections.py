...
def transformed_connections(input_map):
  ...
  original_dict["original_connections"] = [0 for _ in range(len(original_dict["original_locations"]))]
  original_dict["original_connection_names"] = [[] for _ in range(len(original_dict["original_locations"]))]
  
  for k in range(len(input_map["connections"])):
      for j in range(len(input_map["connections"][k]["connected_locations"])):
          
          original_dict["original_connections"][original_dict["original_locations"].index(input_map["connections"][k]["connected_locations"][j])] +=1 
          original_dict["original_connection_names"][original_dict["original_locations"].index(input_map["connections"][k]["connected_locations"][j])].append(input_map["connections"][k]["connected_locations"][:j]
                                                                                                                                                   + input_map["connections"][k]["connected_locations"][j+1:]) 
  for k in range(len(original_dict["original_connection_names"])):
      original_dict["original_connection_names"][k] = sum(original_dict["original_connection_names"][k], [])
